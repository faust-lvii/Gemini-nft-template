...
const NFTDetails = ({ nft }) => {
  if (!nft) {
    return (
      <div className="bg-dark-light rounded-xl p-6 border border-gray-800 text-center">
        <p className="text-gray-400">No NFT selected</p>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className="bg-dark-light rounded-xl border border-gray-800 overflow-hidden"
    >
      {/* NFT Image */}
      <div className={`aspect-square bg-gradient-to-br ${rarityStyle.gradient} relative`}>
        ...
        {rarityStyle.badgeText && (
          <div className={`absolute top-4 right-4 ${rarityStyle.badge} text-sm font-bold px-3 py-1 rounded-md`}>
            {rarityStyle.badgeText}
          </div>
        )}
      </div>
      ...
      <div className="p-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {nft.attributes.map((attr, index) => (
                <div 
                  key={index} 
                  className="bg-dark rounded-lg p-3 border border-gray-800"
                >
        ...
        <div className="mt-6 flex space-x-3">
          {nft.openseaLink && (
            ...
            >
              <svg className="w-5 h-5 mr-2" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
